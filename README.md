...
3. **Story drift detection**  
   Expand the current quest/endgame/shop/temple/sanctum/wizard plus `Medusa`/`Castle`/`Orcus`/`Fort Ludios`/`Vlad`/invocation-portal/wizard-taunt/wizard-amulet-wake/were-full-moon-chat traversal/save-load matrices into broader drift-detection harnesses for economy, religion, branch transitions, and other campaign-critical paths.
