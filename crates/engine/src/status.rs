...
impl StatusEffects {
    #[inline]
    pub fn clamp_timeout(val: u32) -> u32 { val.min(TIMEOUT_MAX) }
...
pub fn is_confused(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.confusion > 0)
}

pub fn is_stunned(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.stun > 0)
}

pub fn is_blind(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.blindness > 0)
}

pub fn is_hallucinating(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.hallucination > 0)
}

pub fn is_levitating(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.levitation > 0)
}

pub fn is_stoning(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.stoning > 0)
}

pub fn is_sliming(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.sliming > 0)
}

pub fn is_sick(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.sick > 0)
}

pub fn is_food_poisoned(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity)
        .is_some_and(|s| s.sick > 0 && (s.sick_type & SICK_VOMITABLE) != 0)
}

pub fn is_strangled(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.strangled > 0)
}

pub fn is_vomiting(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.vomiting > 0)
}

pub fn is_paralyzed(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.paralysis > 0)
}

pub fn is_fumbling(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.fumbling > 0)
}

pub fn is_sleepy(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.sleepy > 0)
}

pub fn has_wounded_legs(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.wounded_legs > 0)
}

pub fn is_phasing(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<StatusEffects>(entity).is_some_and(|s| s.passes_walls > 0)
}

pub fn has_intrinsic_fire_res(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<Intrinsics>(entity).is_some_and(|i| i.fire_resistance)
}
pub fn has_intrinsic_cold_res(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<Intrinsics>(entity).is_some_and(|i| i.cold_resistance)
}
pub fn has_intrinsic_sleep_res(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<Intrinsics>(entity).is_some_and(|i| i.sleep_resistance)
}
pub fn has_intrinsic_shock_res(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<Intrinsics>(entity).is_some_and(|i| i.shock_resistance)
}
pub fn has_intrinsic_poison_res(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<Intrinsics>(entity).is_some_and(|i| i.poison_resistance)
}
pub fn has_intrinsic_disint_res(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<Intrinsics>(entity).is_some_and(|i| i.disintegration_resistance)
}
pub fn has_intrinsic_telepathy(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<Intrinsics>(entity).is_some_and(|i| i.telepathy)
}
pub fn has_intrinsic_teleportitis(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<Intrinsics>(entity).is_some_and(|i| i.teleportitis)
}
pub fn has_intrinsic_teleport_control(world: &GameWorld, entity: Entity) -> bool {
    world.get_component::<Intrinsics>(entity).is_some_and(|i| i.teleport_control)
}
...
fn apply_field(
    world: &mut GameWorld, entity: Entity,
...
pub fn make_confused(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.confusion, |s, v| s.confusion = v, dur) {
        if !was && is { ev.push(EngineEvent::StatusApplied { entity: e, status: StatusEffect::Confused, duration: Some(dur), source: None }); }
        else if was && !is { ev.push(EngineEvent::msg("status-confusion-end")); ev.push(EngineEvent::StatusRemoved { entity: e, status: StatusEffect::Confused }); }
    }
    ev
}
...
pub fn make_stunned(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.stun, |s, v| s.stun = v, dur) {
        if !was && is { ev.push(EngineEvent::StatusApplied { entity: e, status: StatusEffect::Stunned, duration: Some(dur), source: None }); }
        else if was && !is { ev.push(EngineEvent::msg("status-stun-end")); ev.push(EngineEvent::StatusRemoved { entity: e, status: StatusEffect::Stunned }); }
    }
    ev
}
...
pub fn make_blinded(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.blindness, |s, v| s.blindness = v, dur) {
        if !was && is { ev.push(EngineEvent::StatusApplied { entity: e, status: StatusEffect::Blind, duration: Some(dur), source: None }); }
        else if was && !is { ev.push(EngineEvent::msg("status-blindness-end")); ev.push(EngineEvent::StatusRemoved { entity: e, status: StatusEffect::Blind }); }
    }
    ev
}
...
pub fn make_hallucinated(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.hallucination, |s, v| s.hallucination = v, dur) {
        if !was && is { ev.push(EngineEvent::StatusApplied { entity: e, status: StatusEffect::Hallucinating, duration: Some(dur), source: None }); }
        else if was && !is { ev.push(EngineEvent::msg("status-hallucination-end")); ev.push(EngineEvent::StatusRemoved { entity: e, status: StatusEffect::Hallucinating }); }
    }
    ev
}
...
pub fn make_levitating(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.levitation, |s, v| s.levitation = v, dur) {
        if !was && is { ev.push(EngineEvent::StatusApplied { entity: e, status: StatusEffect::Levitating, duration: Some(dur), source: None }); }
        else if was && !is { ev.push(EngineEvent::msg("status-levitation-end")); ev.push(EngineEvent::StatusRemoved { entity: e, status: StatusEffect::Levitating }); }
    }
    ev
}
...
pub fn make_stoned(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.stoning, |s, v| s.stoning = v, dur) {
        if !was && is { ev.push(EngineEvent::StatusApplied { entity: e, status: StatusEffect::Stoning, duration: Some(dur), source: None }); }
        else if was && !is { ev.push(EngineEvent::StatusRemoved { entity: e, status: StatusEffect::Stoning }); }
    }
    ev
}
...
pub fn make_slimed(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.sliming, |s, v| s.sliming = v, dur) {
        if !was && is { ev.push(EngineEvent::StatusApplied { entity: e, status: StatusEffect::Slimed, duration: Some(dur), source: None }); }
        else if was && !is { ev.push(EngineEvent::StatusRemoved { entity: e, status: StatusEffect::Slimed }); }
    }
    ev
}
...
pub fn make_sick(w: &mut GameWorld, e: Entity, dur: u32, sick_type: u8) -> Vec<EngineEvent> {
    let mut ev = Vec::new();
    let r = {
        let mut s = match w.get_component_mut::<StatusEffects>(e) { Some(s) => s, None => return ev };
        let was = s.sick > 0;
        s.sick = StatusEffects::clamp_timeout(dur);
        s.sick_type |= sick_type;
        let is = s.sick > 0;
        if was && !is { s.sick_type = 0; }
        (was, is)
    };
    if !r.0 && r.1 {
        let st = if (sick_type & SICK_VOMITABLE) != 0 { StatusEffect::FoodPoisoned } else { StatusEffect::Sick };
        ev.push(EngineEvent::StatusApplied { entity: e, status: st, duration: Some(dur), source: None });
    } else if r.0 && !r.1 {
        ev.push(EngineEvent::StatusRemoved { entity: e, status: StatusEffect::Sick });
    }
    ev
}
...
pub fn make_vomiting(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.vomiting, |s, v| s.vomiting = v, dur) {
        if !was && is { ev.push(EngineEvent::msg("status-vomiting-start")); }
        else if was && !is { ev.push(EngineEvent::msg("status-vomiting-end")); }
    }
    ev
}
...
pub fn make_paralyzed(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.paralysis, |s, v| s.paralysis = v, dur) {
        if !was && is {
            ...
            ev.push(EngineEvent::StatusRemoved { entity: e, status: StatusEffect::Paralyzed });
        }
    }
    ev
}
...
pub fn make_fumbling(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.fumbling, |s, v| s.fumbling = v, dur) {
        if !was && is { ev.push(EngineEvent::msg("status-fumbling-start")); }
        else if was && !is { ev.push(EngineEvent::msg("status-fumbling-end")); }
    }
    ev
}
...
pub fn make_sleepy(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.sleepy, |s, v| s.sleepy = v, dur) {
        if !was && is { ev.push(EngineEvent::msg("status-sleepy-start")); }
        else if was && !is { ev.push(EngineEvent::msg("status-sleepy-end")); }
    }
    ev
}
...
pub fn wound_legs(w: &mut GameWorld, e: Entity, dur: u32) -> Vec<EngineEvent> {
    ...
    if let Some((was, is)) = apply_field(w, e, |s| s.wounded_legs, |s, v| s.wounded_legs = v, dur) {
        if !was && is { ev.push(EngineEvent::msg("status-wounded-legs-start")); }
        else if was && !is { ev.push(EngineEvent::msg("status-wounded-legs-healed")); }
    }
    ev
}
...
pub fn grant_intrinsic(
    ...
    let mut intr = match world.get_component_mut::<Intrinsics>(entity) {
        Some(i) => i, None => return events,
    };
    match intrinsic {
        crate::hunger::CorpseIntrinsic::FireResistance => {
            if !intr.fire_resistance { intr.fire_resistance = true;
                events.push(EngineEvent::StatusApplied { entity, status: StatusEffect::FireResistance, duration: None, source: None });
                events.push(EngineEvent::msg("intrinsic-fire-res")); }
        }
        crate::hunger::CorpseIntrinsic::ColdResistance => {
            if !intr.cold_resistance { intr.cold_resistance = true;
                events.push(EngineEvent::StatusApplied { entity, status: StatusEffect::ColdResistance, duration: None, source: None });
                events.push(EngineEvent::msg("intrinsic-cold-res")); }
        }
        crate::hunger::CorpseIntrinsic::SleepResistance => {
            if !intr.sleep_resistance { intr.sleep_resistance = true;
                events.push(EngineEvent::StatusApplied { entity, status: StatusEffect::SleepResistance, duration: None, source: None });
                events.push(EngineEvent::msg("intrinsic-sleep-res")); }
        }
        crate::hunger::CorpseIntrinsic::ShockResistance => {
            if !intr.shock_resistance { intr.shock_resistance = true;
                events.push(EngineEvent::StatusApplied { entity, status: StatusEffect::ShockResistance, duration: None, source: None });
                events.push(EngineEvent::msg("intrinsic-shock-res")); }
        }
        crate::hunger::CorpseIntrinsic::PoisonResistance => {
            if !intr.poison_resistance { intr.poison_resistance = true;
                events.push(EngineEvent::StatusApplied { entity, status: StatusEffect::PoisonResistance, duration: None, source: None });
                events.push(EngineEvent::msg("intrinsic-poison-res")); }
        }
        crate::hunger::CorpseIntrinsic::DisintegrationResistance => {
            if !intr.disintegration_resistance { intr.disintegration_resistance = true;
                events.push(EngineEvent::StatusApplied { entity, status: StatusEffect::DisintegrationResistance, duration: None, source: None });
                events.push(EngineEvent::msg("intrinsic-disint-res")); }
        }
        crate::hunger::CorpseIntrinsic::Telepathy => {
            if !intr.telepathy { intr.telepathy = true;
                events.push(EngineEvent::StatusApplied { entity, status: StatusEffect::Telepathy, duration: None, source: None });
                events.push(EngineEvent::msg("intrinsic-telepathy")); }
        }
        crate::hunger::CorpseIntrinsic::Teleportitis => {
            if !intr.teleportitis { intr.teleportitis = true;
                events.push(EngineEvent::msg("intrinsic-teleportitis")); }
        }
        crate::hunger::CorpseIntrinsic::TeleportControl => {
            if !intr.teleport_control { intr.teleport_control = true;
        ...
        crate::hunger::CorpseIntrinsic::Invisibility => {
            ...
            events.push(EngineEvent::StatusApplied { entity, status: StatusEffect::Invisible, duration: Some(250), source: None });
        ...
        crate::hunger::CorpseIntrinsic::SeeInvisible => {
            ...
            events.push(EngineEvent::StatusApplied { entity, status: StatusEffect::SeeInvisible, duration: Some(250), source: None });
...
pub fn tick_status_effects(
    world: &mut GameWorld, entity: Entity, rng: &mut impl Rng,
) -> Vec<EngineEvent> {
    let mut events = Vec::new();
    let mut st: StatusEffects = match world.get_component::<StatusEffects>(entity) {
        Some(s) => (*s).clone(), None => return events,
    };
    ...
    if st.stoning > 0 {
        ...
        match side {
            ...
            StoningSideEffect::ClearVomitingSliming => {
                if st.deaf > 0 && st.deaf < 5 { st.deaf = 5; }
                if st.vomiting > 0 { st.vomiting = 0; }
                if st.sliming > 0 {
                    st.sliming = 0;
                    events.push(EngineEvent::StatusRemoved { entity, status: StatusEffect::Slimed });
                }
            }
    ...
    if st.sliming > 0 {
        ...
        match side {
            ...
            SlimingSideEffect::ClearStoning => {
                if st.stoning > 0 {
                    st.stoning = 0;
                    events.push(EngineEvent::StatusRemoved { entity, status: StatusEffect::Stoning });
                }
            }
            ...
            VomitingSideEffect::AddConfusion(new_conf) => {
                ...
                if clamped > 0 {
                    events.push(EngineEvent::StatusApplied {
                        entity, status: StatusEffect::Confused,
                        duration: Some(clamped), source: None,
                    });
                }
            }
    ...
    macro_rules! dec {
        ($field:ident, $msg:expr, $status:expr) => {
            if st.$field > 0 { st.$field -= 1;
                if st.$field == 0 { events.push(EngineEvent::msg($msg));
                    events.push(EngineEvent::StatusRemoved { entity, status: $status }); } }
        };
    }
    macro_rules! dec_silent { ($field:ident) => { if st.$field > 0 { st.$field -= 1; } }; }
    ...
    dec!(hallucination, "status-hallucination-end", StatusEffect::Hallucinating);
    dec_silent!(deaf);
    dec_silent!(glib);
    dec_silent!(acid_resistance);
    dec_silent!(stone_resistance);
    dec_silent!(protection);
    dec!(levitation, "status-levitation-end", StatusEffect::Levitating);
    if st.invisibility > 0 { st.invisibility -= 1;
        if st.invisibility == 0 { events.push(EngineEvent::msg("status-invisibility-end"));
            events.push(EngineEvent::StatusRemoved { entity, status: StatusEffect::Invisible }); } }
    if st.see_invisible > 0 { st.see_invisible -= 1;
        if st.see_invisible == 0 { events.push(EngineEvent::StatusRemoved { entity, status: StatusEffect::SeeInvisible }); } }

    let mut reset_speed = false;
    if st.speed > 0 { st.speed -= 1;
        if st.speed == 0 { events.push(EngineEvent::msg("status-speed-end"));
            events.push(EngineEvent::StatusRemoved { entity, status: StatusEffect::FastSpeed });
            reset_speed = true; } }
    dec_silent!(vomiting);
    ...
    if st.wounded_legs > 0 { st.wounded_legs -= 1;
        if st.wounded_legs == 0 {
            events.push(EngineEvent::msg("status-wounded-legs-healed"));
        }
    }
    ...
    if st.fumbling > 0 { st.fumbling -= 1;
        if st.fumbling == 0 {
    ...
    if st.stoning > 0 { st.stoning -= 1;
        if st.stoning == 0 {
            events.push(EngineEvent::EntityDied { entity, killer: None, cause: DeathCause::Petrification });
        }
    }
    if st.sliming > 0 { st.sliming -= 1;
        if st.sliming == 0 {
            events.push(EngineEvent::EntityDied { entity, killer: None, cause: DeathCause::Sickness });
        }
    }
    if st.sick > 0 { st.sick -= 1;
        if st.sick == 0 {
            ...
            if ok {
                ...
                events.push(EngineEvent::EntityDied { entity, killer: None, cause: DeathCause::Poisoning });
            }
        }
    }
    if st.strangled > 0 { st.strangled -= 1;
        if st.strangled == 0 {
            events.push(EngineEvent::EntityDied { entity, killer: None, cause: DeathCause::Strangulation });
        }
    }
    ...
    if let Some(mut s) = world.get_component_mut::<StatusEffects>(entity) { *s = st; }
    if reset_speed
        && let Some(mut hsb) = world.get_component_mut::<HeroSpeedBonus>(entity)
            && hsb.0 == HeroSpeed::VeryFast { hsb.0 = HeroSpeed::Normal; }
    events
}
...
pub fn tick_luck(
    ...
    if turn % interval != 0 { return vec![]; }

    let mut pc = match world.get_component_mut::<crate::world::PlayerCombat>(entity) {
        Some(pc) => pc, None => return vec![],
    };

    if pc.luck == base_luck { return vec![]; }
    ...
    let time_luck_effect = if luckstone_blessed { 1i32 }
        else if luckstone_cursed { -1i32 }
        else if has_luckstone { 0i32 }
        else { 0i32 };
...
pub fn tick_spell_protection(
    world: &mut GameWorld,
    entity: Entity,
) -> Vec<EngineEvent> {
    let mut events = Vec::new();
    let mut sp = match world.get_component_mut::<SpellProtection>(entity) {
        Some(sp) => sp, None => return events,
    };
    if sp.countdown == 0 || sp.layers == 0 { return events; }
    sp.countdown -= 1;
...
pub fn tick_hero_counters(
    world: &mut GameWorld,
    entity: Entity,
) -> Vec<EngineEvent> {
    let mut events = Vec::new();
    let mut hc = match world.get_component_mut::<HeroCounters>(entity) {
        Some(hc) => hc, None => return events,
    };
    if hc.creamed > 0 { hc.creamed -= 1; }
    if hc.gallop > 0 {
        hc.gallop -= 1;
        if hc.gallop == 0 {
...
/// v-1==0 actual vomit.
fn vomiting_dialogue(r: u32, confusion: u32, stun: u32, rng: &mut impl Rng)
    -> (Vec<EngineEvent>, VomitingSideEffect)
{
    ...
    match v {
        14 => { msgs.push(EngineEvent::msg("vomiting-mildly-nauseated")); }
        11 => { msgs.push(EngineEvent::msg("vomiting-slightly-confused")); }
        9 => {
            // Add confusion: existing + 2d4
            let add = roll_2d4(rng);
            effect = VomitingSideEffect::AddConfusion(confusion.saturating_add(add));
        }
        8 => { msgs.push(EngineEvent::msg("vomiting-cant-think")); }
        6 => {
        ...
        5 => { msgs.push(EngineEvent::msg("vomiting-incredibly-sick")); }
        2 => { msgs.push(EngineEvent::msg("vomiting-about-to")); }
        0 => {
...
fn sickness_dialogue(r: u32, sick_type: u8) -> Vec<EngineEvent> {
    if r.is_multiple_of(2) { return vec![]; }
    match r / 2 {
        3 => vec![EngineEvent::msg(if (sick_type & SICK_VOMITABLE) != 0 { "sick-sickness-worse" } else { "sick-illness-worse" })],
            ...
            if duration_left <= 0 {
                ...
                if duration_left % 2 != 0 {
                    match duration_left / 2 {
                        ...
                        1 => ExpirationEffect::Message("You are turning into a green slime.".into()),
...
pub fn check_levitation_fall(
    ...
    if is_over_void {
        LevitationEndResult::Fatal { cause: "fell to your death".to_string() }
...
pub fn maybe_confuse_direction(
    confused: bool, stunned: bool, rng: &mut impl Rng,
) -> Option<crate::action::Direction> {
    if stunned || (confused && rng.random_range(0..5u32) == 0) {
        let dirs = [
            crate::action::Direction::North, crate::action::Direction::NorthEast,
            crate::action::Direction::East, crate::action::Direction::SouthEast,
            crate::action::Direction::South, crate::action::Direction::SouthWest,
            crate::action::Direction::West, crate::action::Direction::NorthWest,
        ];
...
mod tests {
    ...
    fn test_rng() -> SmallRng { SmallRng::seed_from_u64(42) }
    fn make_test_world() -> GameWorld { GameWorld::new(Position::new(5, 5)) }

    // I.1: Status effects

    #[test] fn test_status_confusion_apply() {
        let mut w = make_test_world(); let p = w.player();
        assert!(!is_confused(&w, p));
        let ev = make_confused(&mut w, p, 10);
        assert!(is_confused(&w, p));
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::StatusApplied { status: StatusEffect::Confused, .. })));
    }

    #[test] fn test_status_confusion_decay() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_confused(&mut w, p, 3);
        for _ in 0..2 { tick_status_effects(&mut w, p, &mut r); }
        assert!(is_confused(&w, p));
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(!is_confused(&w, p));
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::StatusRemoved { status: StatusEffect::Confused, .. })));
    }

    #[test] fn test_status_confusion_replace() {
        let mut w = make_test_world(); let p = w.player();
    ...
    #[test] fn test_status_confusion_cure() {
        let mut w = make_test_world(); let p = w.player();
        make_confused(&mut w, p, 10);
    ...
    #[test] fn test_status_blindness_apply_decay() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_blinded(&mut w, p, 2); assert!(is_blind(&w, p));
        tick_status_effects(&mut w, p, &mut r); assert!(is_blind(&w, p));
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(!is_blind(&w, p));
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::StatusRemoved { status: StatusEffect::Blind, .. })));
    }

    #[test] fn test_status_blindness_cure() {
        let mut w = make_test_world(); let p = w.player();
        make_blinded(&mut w, p, 100); make_blinded(&mut w, p, 0); assert!(!is_blind(&w, p));
    }

    #[test] fn test_status_stun_apply_decay() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_stunned(&mut w, p, 2);
        tick_status_effects(&mut w, p, &mut r);
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(!is_stunned(&w, p));
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::StatusRemoved { status: StatusEffect::Stunned, .. })));
    }

    #[test] fn test_status_hallucination_apply_decay() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_hallucinated(&mut w, p, 3);
        tick_status_effects(&mut w, p, &mut r); tick_status_effects(&mut w, p, &mut r);
        assert!(is_hallucinating(&w, p));
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(!is_hallucinating(&w, p));
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::StatusRemoved { status: StatusEffect::Hallucinating, .. })));
    }

    #[test] fn test_status_levitation_apply_decay() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_levitating(&mut w, p, 2);
        tick_status_effects(&mut w, p, &mut r);
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(!is_levitating(&w, p));
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::StatusRemoved { status: StatusEffect::Levitating, .. })));
    }

    #[test] fn test_status_levitation_queryable() {
        let mut w = make_test_world(); let p = w.player();
        assert!(!is_levitating(&w, p));
    ...
    #[test] fn test_status_stoning_death() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_stoned(&mut w, p, STONING_INITIAL);
        for _ in 0..4 { let ev = tick_status_effects(&mut w, p, &mut r);
            assert!(!ev.iter().any(|e| matches!(e, EngineEvent::EntityDied { .. }))); }
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::EntityDied { cause: DeathCause::Petrification, .. })));
    }

    #[test] fn test_status_stoning_cure() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_stoned(&mut w, p, STONING_INITIAL);
        tick_status_effects(&mut w, p, &mut r); tick_status_effects(&mut w, p, &mut r);
        make_stoned(&mut w, p, 0); assert!(!is_stoning(&w, p));
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(!ev.iter().any(|e| matches!(e, EngineEvent::EntityDied { .. })));
    }

    #[test] fn test_status_stoning_dialogue() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_stoned(&mut w, p, STONING_INITIAL);
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "stoning-slowing-down")));
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "stoning-limbs-stiffening")));
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "stoning-limbs-stone")));
    }

    #[test] fn test_status_sliming_death() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_slimed(&mut w, p, SLIMING_INITIAL);
        for _ in 0..9 { let ev = tick_status_effects(&mut w, p, &mut r);
            assert!(!ev.iter().any(|e| matches!(e, EngineEvent::EntityDied { .. }))); }
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::EntityDied { .. })));
    }

    #[test] fn test_status_sliming_cure() {
        let mut w = make_test_world(); let p = w.player();
        make_slimed(&mut w, p, SLIMING_INITIAL); make_slimed(&mut w, p, 0);
        assert!(!is_sliming(&w, p));
    }

    #[test] fn test_status_sliming_dialogue_odd() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_slimed(&mut w, p, SLIMING_INITIAL);
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(!ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key.starts_with("sliming-"))));
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "sliming-turning-green")));
    }

    #[test] fn test_status_food_poisoning() {
        let mut w = make_test_world(); let p = w.player(); let mut r = SmallRng::seed_from_u64(999);
        make_sick(&mut w, p, 3, SICK_VOMITABLE);
        assert!(is_food_poisoned(&w, p));
        let mut ok = false;
        for _ in 0..5 { let ev = tick_status_effects(&mut w, p, &mut r);
    ...
    #[test] fn test_status_disease_fatal() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_sick(&mut w, p, 2, SICK_NONVOMITABLE);
        tick_status_effects(&mut w, p, &mut r);
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::EntityDied { cause: DeathCause::Poisoning, .. })));
    }

    #[test] fn test_status_sick_dual_type() {
        let mut w = make_test_world(); let p = w.player();
        make_sick(&mut w, p, 10, SICK_VOMITABLE);
        make_sick(&mut w, p, 10, SICK_NONVOMITABLE);
        assert_eq!(w.get_component::<StatusEffects>(p).unwrap().sick_type, SICK_ALL);
    ...
    #[test] fn test_status_sick_full_cure() {
        let mut w = make_test_world(); let p = w.player();
        make_sick(&mut w, p, 10, SICK_VOMITABLE);
        let ev = cure_sick(&mut w, p, SICK_VOMITABLE);
        assert!(!is_sick(&w, p));
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::StatusRemoved { status: StatusEffect::Sick, .. })));
    }

    #[test] fn test_status_vomiting_decay() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
    ...
    #[test] fn test_status_timeout_clamp() {
        let mut w = make_test_world(); let p = w.player();
        make_confused(&mut w, p, TIMEOUT_MAX + 1000);
        assert_eq!(w.get_component::<StatusEffects>(p).unwrap().confusion, TIMEOUT_MAX);
    }

    #[test] fn test_status_incr_clamp() {
        let mut v: u32 = TIMEOUT_MAX - 5;
        StatusEffects::incr_timeout(&mut v, 100);
        assert_eq!(v, TIMEOUT_MAX);
    }

    #[test] fn test_status_multiple_simultaneous() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_confused(&mut w, p, 5); make_stunned(&mut w, p, 3); make_blinded(&mut w, p, 7);
        for _ in 0..3 { tick_status_effects(&mut w, p, &mut r); }
        assert!(is_confused(&w, p)); assert!(!is_stunned(&w, p)); assert!(is_blind(&w, p));
        for _ in 0..2 { tick_status_effects(&mut w, p, &mut r); }
        assert!(!is_confused(&w, p)); assert!(is_blind(&w, p));
        for _ in 0..2 { tick_status_effects(&mut w, p, &mut r); }
        assert!(!is_blind(&w, p));
    }

    #[test] fn test_status_confdir_randomization() {
        let mut r = test_rng();
        for _ in 0..100 { assert!(maybe_confuse_direction(false, false, &mut r).is_none()); }
        for _ in 0..10 { assert!(maybe_confuse_direction(false, true, &mut r).is_some()); }
        let mut n = 0; let t = 5000;
        for _ in 0..t { if maybe_confuse_direction(true, false, &mut r).is_some() { n += 1; } }
    ...
    #[test] fn test_intrinsic_fire_res() {
        let mut w = make_test_world(); let p = w.player();
        let ev = grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::FireResistance);
        assert!(has_intrinsic_fire_res(&w, p));
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::StatusApplied { status: StatusEffect::FireResistance, .. })));
    }

    #[test] fn test_intrinsic_fire_res_no_dup() {
        let mut w = make_test_world(); let p = w.player();
    ...
    #[test] fn test_intrinsic_cold_res() {
        let mut w = make_test_world(); let p = w.player();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::ColdResistance);
        assert!(has_intrinsic_cold_res(&w, p));
    }

    #[test] fn test_intrinsic_sleep_res() {
        let mut w = make_test_world(); let p = w.player();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::SleepResistance);
        assert!(has_intrinsic_sleep_res(&w, p));
    }

    #[test] fn test_intrinsic_shock_res() {
        let mut w = make_test_world(); let p = w.player();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::ShockResistance);
        assert!(has_intrinsic_shock_res(&w, p));
    }

    #[test] fn test_intrinsic_poison_res() {
        let mut w = make_test_world(); let p = w.player();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::PoisonResistance);
        assert!(has_intrinsic_poison_res(&w, p));
    }

    #[test] fn test_intrinsic_disint_res() {
        let mut w = make_test_world(); let p = w.player();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::DisintegrationResistance);
        assert!(has_intrinsic_disint_res(&w, p));
    }

    #[test] fn test_intrinsic_telepathy() {
        let mut w = make_test_world(); let p = w.player();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::Telepathy);
        assert!(has_intrinsic_telepathy(&w, p));
    }

    #[test] fn test_intrinsic_teleportitis() {
        let mut w = make_test_world(); let p = w.player();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::Teleportitis);
        assert!(has_intrinsic_teleportitis(&w, p));
    }

    #[test] fn test_intrinsic_teleport_control() {
        let mut w = make_test_world(); let p = w.player();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::TeleportControl);
        assert!(has_intrinsic_teleport_control(&w, p));
    }

    #[test] fn test_intrinsic_strength_giant() {
        let mut w = make_test_world(); let p = w.player();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::Strength);
        assert_eq!(w.get_component::<Intrinsics>(p).unwrap().giant_strength_gained, 1);
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::Strength);
        assert_eq!(w.get_component::<Intrinsics>(p).unwrap().giant_strength_gained, 2);
    }

    #[test] fn test_intrinsic_temp_acid_res() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::AcidResistance { duration: 10 });
        assert_eq!(w.get_component::<StatusEffects>(p).unwrap().acid_resistance, 10);
        for _ in 0..10 { tick_status_effects(&mut w, p, &mut r); }
        assert_eq!(w.get_component::<StatusEffects>(p).unwrap().acid_resistance, 0);
    }

    #[test] fn test_intrinsic_temp_stone_res() {
        let mut w = make_test_world(); let p = w.player();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::StoneResistance { duration: 8 });
        assert_eq!(w.get_component::<StatusEffects>(p).unwrap().stone_resistance, 8);
    }

    #[test] fn test_intrinsic_temp_res_stacks() {
        let mut w = make_test_world(); let p = w.player();
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::AcidResistance { duration: 5 });
        grant_intrinsic(&mut w, p, &crate::hunger::CorpseIntrinsic::AcidResistance { duration: 7 });
        assert_eq!(w.get_component::<StatusEffects>(p).unwrap().acid_resistance, 12);
    }

    #[test] fn test_status_sick_dialogue_food() {
        let ev = sickness_dialogue(7, SICK_VOMITABLE);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "sick-sickness-worse")));
    }

    #[test] fn test_status_sick_dialogue_disease() {
        let ev = sickness_dialogue(7, SICK_NONVOMITABLE);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "sick-illness-worse")));
    }

    #[test] fn test_status_no_components_safe() {
        let mut w = make_test_world();
        let bare = w.spawn((crate::world::Positioned(Position::new(1, 1)),));
        assert!(!is_confused(&w, bare)); assert!(!is_stunned(&w, bare));
    }

    #[test] fn test_status_zero_duration_noop() {
        let mut w = make_test_world(); let p = w.player();
        let ev = make_confused(&mut w, p, 0);
        assert!(ev.is_empty()); assert!(!is_confused(&w, p));
    }

    #[test] fn test_status_strangled_death() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        { let mut s = w.get_component_mut::<StatusEffects>(p).unwrap(); s.strangled = STRANGLED_INITIAL; }
        for _ in 0..4 { let ev = tick_status_effects(&mut w, p, &mut r);
            assert!(!ev.iter().any(|e| matches!(e, EngineEvent::EntityDied { .. }))); }
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::EntityDied { cause: DeathCause::Strangulation, .. })));
    }

    #[test] fn test_status_speed_expiry() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        { let mut s = w.get_component_mut::<StatusEffects>(p).unwrap(); s.speed = 2; }
        { let mut h = w.get_component_mut::<HeroSpeedBonus>(p).unwrap(); h.0 = HeroSpeed::VeryFast; }
        tick_status_effects(&mut w, p, &mut r);
        assert_eq!(w.get_component::<HeroSpeedBonus>(p).unwrap().0, HeroSpeed::VeryFast);
        let ev = tick_status_effects(&mut w, p, &mut r);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::StatusRemoved { status: StatusEffect::FastSpeed, .. })));
        assert_eq!(w.get_component::<HeroSpeedBonus>(p).unwrap().0, HeroSpeed::Normal);
    }
    ...
    fn test_hallucinate_names_different() {
        ...
        assert!(monster_differed, "hallucinated monster name should differ from real name");
        assert!(item_differed, "hallucinated item name should differ from real name");
    }
    ...
    // Strangling dialogue
    #[test] fn test_choke_dialogue_emits_messages() {
        let mut r = SmallRng::seed_from_u64(0);
        for stage in 1..=5 {
            let ev = choke_dialogue(stage, &mut r);
            assert!(!ev.is_empty(), "choke dialogue should emit message at stage {stage}");
        }
    ...
    #[test] fn test_stoning_stage5_clears_speed() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        { let mut s = w.get_component_mut::<StatusEffects>(p).unwrap(); s.speed = 50; }
        make_stoned(&mut w, p, STONING_INITIAL);
        tick_status_effects(&mut w, p, &mut r);
    ...
    #[test] fn test_stoning_stage3_paralyzes() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_stoned(&mut w, p, STONING_INITIAL);
    ...
    #[test] fn test_stoning_stage2_clears_vomiting_sliming() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_stoned(&mut w, p, STONING_INITIAL);
    ...
    #[test] fn test_sliming_stage_clears_speed() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        { let mut s = w.get_component_mut::<StatusEffects>(p).unwrap(); s.speed = 50; }
    ...
    #[test] fn test_sliming_clears_stoning() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_slimed(&mut w, p, SLIMING_INITIAL);
        make_stoned(&mut w, p, STONING_INITIAL);
        // Fast-forward to t=3 (i=1): need 7 ticks from t=10
        for _ in 0..7 { tick_status_effects(&mut w, p, &mut r); }
    ...
    #[test] fn test_vomiting_adds_confusion() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_vomiting(&mut w, p, 10); // v=10, v-1=9 on first tick
        tick_status_effects(&mut w, p, &mut r);
        assert!(w.get_component::<StatusEffects>(p).unwrap().confusion > 0,
            "vomiting at v-1=9 should add confusion");
    }

    // Vomiting side effects: v-1==6 adds stun AND confusion
    #[test] fn test_vomiting_adds_stun_and_confusion() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        make_vomiting(&mut w, p, 7); // v=7, v-1=6 on first tick
        tick_status_effects(&mut w, p, &mut r);
        let st = w.get_component::<StatusEffects>(p).unwrap();
        assert!(st.stun > 0, "vomiting at v-1=6 should add stun");
        assert!(st.confusion > 0, "vomiting at v-1=6 should add confusion (FALLTHROUGH)");
    }

    // Fumbling self-cycling timer
    #[test] fn test_fumbling_self_cycling() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        { let mut s = w.get_component_mut::<StatusEffects>(p).unwrap(); s.fumbling = 2; }
        ...
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "status-fumble-trip")));
    }

    // Sleepy self-cycling timer
    #[test] fn test_sleepy_self_cycling() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        { let mut s = w.get_component_mut::<StatusEffects>(p).unwrap(); s.sleepy = 1; }
        let ev = tick_status_effects(&mut w, p, &mut r);
        let st = w.get_component::<StatusEffects>(p).unwrap();
        assert!(st.sleepy > 0, "sleepy should reset on expiry");
        assert!(st.paralysis > 0, "sleepy expiry should cause paralysis");
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "status-fall-asleep")));
    }

    // Wounded legs recovery
    #[test] fn test_wounded_legs_recovery() {
        let mut w = make_test_world(); let p = w.player(); let mut r = test_rng();
        ...
        assert!(!has_wounded_legs(&w, p));
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "status-wounded-legs-healed")));
    }

    // Levitation dialogue
    #[test] fn test_levitation_dialogue_at_3() {
        // t=3, i=(3-1)/2=1, odd → "wobble"
        let ev = levitation_dialogue(3);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "levitation-wobble")));
    }

    #[test] fn test_levitation_dialogue_even_silent() {
        // t=4, even → no message
    ...
    #[test] fn test_sleep_dialogue_yawn_at_4() {
        let ev = sleep_dialogue(4);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "sleepy-yawn")));
    }

    #[test] fn test_sleep_dialogue_silent_at_3() {
        assert!(sleep_dialogue(3).is_empty());
    }

    // Phaze dialogue
    #[test] fn test_phaze_dialogue_at_3() {
        // t=3, odd, i=3/2=1 → "feeling flabby"
        let ev = phaze_dialogue(3);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "phaze-feeling-flabby")));
    }

    // Spell protection dissipation
    #[test] fn test_spell_protection_dissipation() {
        let mut w = make_test_world(); let p = w.player();
        let _ = w.ecs_mut().insert_one(p, SpellProtection { layers: 2, countdown: 1, interval: 10 });
    ...
    #[test] fn test_spell_protection_final_layer() {
        let mut w = make_test_world(); let p = w.player();
        let _ = w.ecs_mut().insert_one(p, SpellProtection { layers: 1, countdown: 1, interval: 10 });
        let ev = tick_spell_protection(&mut w, p);
    ...
    // Hero counters
    #[test] fn test_hero_counters_gallop() {
        let mut w = make_test_world(); let p = w.player();
        let _ = w.ecs_mut().insert_one(p, HeroCounters { creamed: 3, gallop: 1 });
        let ev = tick_hero_counters(&mut w, p);
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "steed-stops-galloping")));
    ...
    // Burn away slime
    #[test] fn test_burn_away_slime() {
        let mut w = make_test_world(); let p = w.player();
        make_slimed(&mut w, p, SLIMING_INITIAL);
        assert!(is_sliming(&w, p));
        let ev = burn_away_slime(&mut w, p);
        assert!(!is_sliming(&w, p));
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "slime-burned-away")));
    }

    #[test] fn test_burn_away_slime_noop_if_not_slimed() {
        let mut w = make_test_world(); let p = w.player();
        let ev = burn_away_slime(&mut w, p);
        assert!(ev.is_empty());
    }
    ...
    // Luck decay
    #[test] fn test_luck_decay_toward_base() {
        let mut w = make_test_world(); let p = w.player();
        { let mut pc = w.get_component_mut::<crate::world::PlayerCombat>(p).unwrap(); pc.luck = 5; }
        // At turn 600, no luckstone
        tick_luck(&mut w, p, 600, 0, false, false, false, false);
        assert_eq!(w.get_component::<crate::world::PlayerCombat>(p).unwrap().luck, 4);
    }

    #[test] fn test_luck_no_decay_with_uncursed_luckstone() {
        let mut w = make_test_world(); let p = w.player();
        { let mut pc = w.get_component_mut::<crate::world::PlayerCombat>(p).unwrap(); pc.luck = 5; }
        tick_luck(&mut w, p, 600, 0, true, false, false, false);
        assert_eq!(w.get_component::<crate::world::PlayerCombat>(p).unwrap().luck, 5);
    }

    #[test] fn test_luck_decay_with_cursed_luckstone() {
        let mut w = make_test_world(); let p = w.player();
        { let mut pc = w.get_component_mut::<crate::world::PlayerCombat>(p).unwrap(); pc.luck = 5; }
        // Cursed luckstone: positive luck decays, negative doesn't recover
    ...
    #[test] fn test_luck_recovery_with_blessed_luckstone() {
        let mut w = make_test_world(); let p = w.player();
        { let mut pc = w.get_component_mut::<crate::world::PlayerCombat>(p).unwrap(); pc.luck = -3; }
        // Blessed luckstone: negative luck recovers
        tick_luck(&mut w, p, 600, 0, true, false, true, false);
        assert_eq!(w.get_component::<crate::world::PlayerCombat>(p).unwrap().luck, -2);
    }

    #[test] fn test_luck_no_decay_off_interval() {
        let mut w = make_test_world(); let p = w.player();
        { let mut pc = w.get_component_mut::<crate::world::PlayerCombat>(p).unwrap(); pc.luck = 5; }
    ...
    #[test] fn test_luck_faster_decay_with_amulet() {
        let mut w = make_test_world(); let p = w.player();
        { let mut pc = w.get_component_mut::<crate::world::PlayerCombat>(p).unwrap(); pc.luck = 5; }
    ...
    fn test_expiration_stoning_complete_at_zero() {
        assert_eq!(expiration_effect("stoning", 0), ExpirationEffect::StoningComplete);
    }

    #[test]
    fn test_expiration_stoning_progressive_warnings() {
        assert!(matches!(expiration_effect("stoning", 5), ExpirationEffect::Message(m) if m.contains("slowing down")));
        assert!(matches!(expiration_effect("stoning", 4), ExpirationEffect::Message(m) if m.contains("stiffening")));
        assert!(matches!(expiration_effect("stoning", 3), ExpirationEffect::Message(m) if m.contains("turned to stone")));
        assert!(matches!(expiration_effect("stoning", 2), ExpirationEffect::Message(m) if m.contains("turned to stone")));
        assert!(matches!(expiration_effect("stoning", 1), ExpirationEffect::Message(m) if m.contains("statue")));
    }

    #[test]
    fn test_expiration_sliming_complete_at_zero() {
        assert_eq!(expiration_effect("sliming", 0), ExpirationEffect::SlimingComplete);
    }

    #[test]
    fn test_expiration_sliming_progressive_warnings() {
        // Messages only on odd values; even values are NoEffect
        assert!(matches!(expiration_effect("sliming", 9), ExpirationEffect::Message(m) if m.contains("green")));
        assert_eq!(expiration_effect("sliming", 8), ExpirationEffect::NoEffect);
        assert!(matches!(expiration_effect("sliming", 7), ExpirationEffect::Message(m) if m.contains("oozy")));
        assert!(matches!(expiration_effect("sliming", 5), ExpirationEffect::Message(m) if m.contains("peel")));
        assert!(matches!(expiration_effect("sliming", 3), ExpirationEffect::Message(m) if m.contains("turning into")));
        assert!(matches!(expiration_effect("sliming", 1), ExpirationEffect::Message(m) if m.contains("become")));
    }

    #[test]
    fn test_expiration_sickness_kills_at_zero() {
        assert_eq!(expiration_effect("sick", 0), ExpirationEffect::SicknessKills);
    }

    #[test]
    fn test_expiration_sickness_progression() {
        // Messages at odd values with i = t/2
        assert!(matches!(expiration_effect("sick", 7), ExpirationEffect::Message(m) if m.contains("worse")));
        assert_eq!(expiration_effect("sick", 6), ExpirationEffect::NoEffect);
        assert!(matches!(expiration_effect("sick", 5), ExpirationEffect::Message(m) if m.contains("severe")));
        assert!(matches!(expiration_effect("sick", 3), ExpirationEffect::Message(m) if m.contains("Death's door")));
    }

    #[test]
    fn test_expiration_strangulation_kills_at_zero() {
        assert_eq!(expiration_effect("strangled", 0), ExpirationEffect::StrangulationKills);
    }

    #[test]
    fn test_expiration_strangulation_warnings() {
        assert!(matches!(expiration_effect("strangled", 5), ExpirationEffect::Message(m) if m.contains("hard to breathe")));
        assert!(matches!(expiration_effect("strangled", 4), ExpirationEffect::Message(m) if m.contains("gasping")));
        assert!(matches!(expiration_effect("strangled", 3), ExpirationEffect::Message(m) if m.contains("no longer breathe")));
        assert!(matches!(expiration_effect("strangled", 2), ExpirationEffect::Message(m) if m.contains("turning blue")));
        assert!(matches!(expiration_effect("strangled", 1), ExpirationEffect::Message(m) if m.contains("suffocate")));
    }
    ...
    fn test_expiration_speed_normal() {
        ...
        assert_eq!(expiration_effect("very_fast", 0), ExpirationEffect::SpeedNormal);
        assert_eq!(expiration_effect("speed", 5), ExpirationEffect::NoEffect);
    }

    #[test]
    fn test_expiration_blindness_restored() {
        assert_eq!(expiration_effect("blind", 0), ExpirationEffect::SightRestored);
        assert_eq!(expiration_effect("blindness", 0), ExpirationEffect::SightRestored);
    }

    #[test]
    fn test_expiration_confusion_clears() {
        assert_eq!(expiration_effect("confused", 0), ExpirationEffect::ConfusionClears);
        assert_eq!(expiration_effect("confusion", 0), ExpirationEffect::ConfusionClears);
    }

    #[test]
    fn test_expiration_stun_clears() {
        assert_eq!(expiration_effect("stunned", 0), ExpirationEffect::StunClears);
        assert_eq!(expiration_effect("stun", 0), ExpirationEffect::StunClears);
    }

    #[test]
    fn test_expiration_hallucination_ends() {
        assert_eq!(expiration_effect("hallucinating", 0), ExpirationEffect::HallucinationEnds);
        assert_eq!(expiration_effect("hallucination", 0), ExpirationEffect::HallucinationEnds);
    }

    #[test]
    fn test_expiration_levitation_fall() {
        assert_eq!(expiration_effect("levitation", 0), ExpirationEffect::FallFromLevitation { height: 0 });
        assert_eq!(expiration_effect("levitation", 5), ExpirationEffect::NoEffect);
    }

    #[test]
    fn test_expiration_invisibility_ends() {
        assert_eq!(expiration_effect("invisibility", 0), ExpirationEffect::BecomeVisible);
    }

    #[test]
    fn test_expiration_polymorph_reverts() {
        assert_eq!(expiration_effect("polymorph", 0), ExpirationEffect::RevertPolymorph);
    }

    #[test]
    fn test_expiration_protection_fades() {
        assert!(matches!(expiration_effect("protection", 0), ExpirationEffect::Message(m) if m.contains("protected")));
    }

    #[test]
    fn test_expiration_wounded_legs_heals() {
        assert!(matches!(expiration_effect("wounded_legs", 0), ExpirationEffect::Message(m) if m.contains("legs")));
    }

    #[test]
    fn test_expiration_unknown_property() {
        assert_eq!(expiration_effect("nonexistent", 0), ExpirationEffect::NoEffect);
        assert_eq!(expiration_effect("nonexistent", 5), ExpirationEffect::NoEffect);
    }
    ...
    fn test_tick_timed_properties_fatal_at_zero() {
        ...
        assert!(effects.iter().any(|(_, e)| *e == ExpirationEffect::StoningComplete));
    }
    ...
    fn test_tick_timed_properties_non_fatal_expiry() {
        ...
        let effects = tick_timed_properties(&mut props);
        assert!(props.is_empty());
        assert!(effects.iter().any(|(_, e)| *e == ExpirationEffect::SpeedNormal));
    }

    #[test]
    fn test_egg_hatch_at_threshold() {
        let result = check_egg_hatch(100, 100, "cockatrice");
        assert!(matches!(result, Some(ExpirationEffect::EggHatches { monster_type }) if monster_type == "cockatrice"));
    }

    #[test]
    fn test_egg_hatch_past_threshold() {
        let result = check_egg_hatch(150, 100, "snake");
        assert!(matches!(result, Some(ExpirationEffect::EggHatches { monster_type }) if monster_type == "snake"));
    }
    ...
    fn test_sickness_warns_before_death() {
        ...
        assert!(ev.iter().any(|e| matches!(e, EngineEvent::Message { key, .. } if key == "sick-deaths-door")),
    ...
    fn test_levitation_fall_damage() {
        ...
        match result {
            LevitationEndResult::Landing { damage } => {
                assert!((1..=6).contains(&damage), "fall damage should be 1-6, got {}", damage);
            }
    ...
    fn test_levitation_fall_over_lava_fatal() {
        ...
        assert_eq!(result, LevitationEndResult::FallIntoLava,
    ...
    fn test_levitation_fall_over_void_fatal() {
        ...
        assert!(matches!(result, LevitationEndResult::Fatal { .. }),
