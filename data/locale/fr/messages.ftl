...
opt-on = OUI
opt-off = NON
ui-options-game-title = Paramètres de jeu ({ $count } options)
...
ui-offer-prompt = Offre>
ui-confirm-quit = Vraiment quitter ?
ui-save-failed = Échec de la sauvegarde : { $error }
ui-save-create-dir-failed = Impossible de créer le répertoire de sauvegarde : { $error }
ui-save-load-warning = Avertissement : impossible de charger la sauvegarde : { $error }
ui-save-load-new-game = Démarrage d'une nouvelle partie.
ui-commands-title = Commandes
ui-here-commands-title = Commandes pour ici
ui-there-commands-title = Commandes pour la cible
ui-direction-prompt = Dans quelle direction ?
ui-direction-prompt-optional = Dans quelle direction ? (Échap pour aucune)
ui-direction-prompt-run = Courir dans quelle direction ?
...
ui-count-prefix = Compteur
ui-recording-start = Enregistrement de la session dans : { $path }
ui-recording-saved = Session enregistrée dans : { $path }
ui-recording-save-warning = Avertissement : impossible d'enregistrer la session : { $error }
...
ui-text-status-line = Niveau:{ $depth }  { $hp }  Tour:{ $turn }  Pos:{ $pos }  [hjklyubn=deplacement .=attendre <=monter >=descendre q=quitter ?=aide]
ui-startup-loaded-filesystem = { $monsters } monstres et { $objects } objets chargés depuis { $path }
ui-startup-loaded-embedded = { $monsters } monstres et { $objects } objets chargés depuis les ressources intégrées
ui-startup-language = Langue : { $code } ({ $name })
ui-restored-save = Partie restaurée : tour { $turn }, profondeur { $depth }.

option-label-autopickup = Ramassage automatique
...
wizard-taunt-last-breath = Savoure ton souffle, { $insult }, ce sera ton dernier !
wizard-taunt-return = Je reviendrai.
wizard-taunt-back = Je serai de retour.
