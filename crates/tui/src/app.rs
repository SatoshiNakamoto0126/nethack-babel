...
use crate::input::{
    complete_extended_command, key_needs_prompt, map_direction_key,
    map_extended_command, map_key, DirectionCommand, ItemCommand,
    ItemDirectionCommand, PromptKind,
};
use crate::port::{
    InputEvent, InputKeyCode, MapView, MessageUrgency, StatusLine, WindowPort,
};
...
impl App {
    ...
    pub fn display_pending_messages(
        &mut self,
        port: &mut impl WindowPort,
    ) {
        let pending: Vec<MessageEntry> =
            self.messages.pending().to_vec();
            ...
            match event {
                ...
                InputEvent::Key {
                    ...
                    if let Some(completed) =
                        complete_extended_command(&buffer)
                    {
                        buffer = completed;
                        port.show_message(
                            &format!("# {}", buffer),
    ...
    pub fn update_inventory_letters(
        &mut self,
        items: impl IntoIterator<Item = (Entity, char)>,
    ) {
        ...
        loop {
            ...
            match event {
                InputEvent::Key { code, modifiers } => {
                    ...
                    let key_event =
                        crossterm::event::KeyEvent::new(ct_code, ct_mods);
    ...
    fn build_item_action(
        ...
        if letter == '-' {
            return match command {
                ItemCommand::Wield => {
                    ...
                    port.show_message(
                        &self.messages_i18n.empty_handed,
                        MessageUrgency::Normal,
                    );
                    None
                }
        ...
        let entity = match self.inventory_letters.get(&letter) {
            ...
            None => {
                port.show_message(
                    &self.messages_i18n.no_such_item,
                    MessageUrgency::Normal,
                );
                return None;
            }
        ...
        loop {
            ...
            match event {
                InputEvent::Key { code, modifiers } => {
                    ...
                    if let Some(prompt_kind) =
                        key_needs_prompt(key_event)
                    {
                        match prompt_kind {
                            PromptKind::Item { prompt, command } => {
                                if let Some(letter) = self
                                    .prompt_inventory_item(
                                        port, prompt,
                                    )
...
mod tests {
    use super::*;
    use crate::port::{
        InputEvent, InputKeyCode, InputModifiers, Menu, MenuResult,
    };
    ...
    impl WindowPort for MockPort {
        ...
        fn show_text(&mut self, _title: &str, _content: &str) {}
        fn get_key(&mut self) -> InputEvent {
            self.keys
                .pop_front()
                .unwrap_or(InputEvent::None)
        }
        fn ask_direction(
            &mut self,
            _prompt: &str,
    ...
    fn count_prefix_can_be_set_and_read() {
        ...
        app.count_prefix =
            Some(app.count_prefix.unwrap_or(0).saturating_mul(10));
    ...
    fn count_prefix_saturating_overflow() {
        ...
        for _ in 0..15 {
            ...
            app.count_prefix =
                Some(current.saturating_mul(10).saturating_add(9));
        }
    ...
    fn prompt_direction_returns_vi_key() {
        ...
        let result =
            app.prompt_direction(&mut port, "In what direction?");
    ...
    fn prompt_direction_all_vi_keys() {
        ...
        for (key, expected_dir) in cases {
            let app = App::new();
            let mut port =
                MockPort::new(vec![MockPort::key(key)]);
            let result =
                app.prompt_direction(&mut port, "Direction?");
            assert_eq!(
                result,
                Some(expected_dir),
                "key '{}' should map to {:?}",
    ...
    fn build_item_action_valid_letter() {
        ...
        let mut port = MockPort::new(vec![]);
        let result =
            app.build_item_action(&mut port, 'a', ItemCommand::Wield);
        assert!(matches!(
            result,
    ...
    fn build_item_action_unknown_letter() {
        ...
        let mut port = MockPort::new(vec![]);
        let result =
            app.build_item_action(&mut port, 'z', ItemCommand::Drop);
    ...
    fn build_item_action_dash_wield_is_none() {
        ...
        let mut port = MockPort::new(vec![]);
        let result =
            app.build_item_action(&mut port, '-', ItemCommand::Wield);
        assert!(result.is_none());
        assert!(
            port.messages.iter().any(|m| m.contains("empty handed"))
        );
    }
    ...
    fn build_item_action_dash_non_wield_is_none() {
        ...
        let mut port = MockPort::new(vec![]);
        let result =
            app.build_item_action(&mut port, '-', ItemCommand::Drop);
        assert!(result.is_none());
        assert!(
            port.messages.iter().any(|m| m.contains("Never mind"))
        );
    }
    ...
    fn build_item_action_star_is_none() {
        ...
        let mut port = MockPort::new(vec![]);
        let result =
            app.build_item_action(&mut port, '*', ItemCommand::Wear);
        assert!(result.is_none());
    }
    ...
    fn build_item_action_all_commands() {
        ...
        let commands_and_expected: Vec<(
            ItemCommand,
            fn(Entity) -> PlayerAction,
        )> = vec![
            ...
            (ItemCommand::TakeOff, |e| {
                PlayerAction::TakeOff { item: e }
            }),
            (ItemCommand::PutOn, |e| PlayerAction::PutOn { item: e }),
            (ItemCommand::Remove, |e| {
                PlayerAction::Remove { item: e }
            }),
        ...
        for (command, _make_expected) in &commands_and_expected {
            let mut port = MockPort::new(vec![]);
            let result =
                app.build_item_action(&mut port, 'x', *command);
            assert!(
                result.is_some(),
    ...
    fn build_direction_action_open() {
        let action = App::build_direction_action(
            Direction::North,
            DirectionCommand::Open,
        );
        assert!(matches!(
    ...
    fn update_inventory_letters_replaces_map() {
        ...
        app.update_inventory_letters(vec![
            (e1, 'a'),
            (e2, 'b'),
        ]);
