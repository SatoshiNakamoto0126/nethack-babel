# NetHack Babel（迷宫通灵塔）

**NetHack 3.7 的现代 Rust 重新实现，内建多语言支持。**

[English README](README.md)

[![License: NGPL](https://img.shields.io/badge/License-NGPL-blue.svg)](LICENSE)
![Rust: nightly](https://img.shields.io/badge/Rust-nightly-orange.svg)
![Tests: 4677](https://img.shields.io/badge/tests-4677_passing-brightgreen.svg)
![LOC: 211K](https://img.shields.io/badge/LOC-211K_Rust-informational.svg)

## 代码规模对比：C 原版 vs Rust 重制版

| 指标 | C NetHack 3.7 | Rust Babel | 备注 |
|------|--------------|------------|------|
| **语言** | C + Lua | Rust | 纯 Rust，无 C 依赖 |
| **代码行数** | 424K（`.c/.h`）+ 24K（`.lua`） | 211K（`.rs`）+ 14K 本地化 + 26K TOML | Rust 把逻辑、数据、i18n 明确拆开，而不是把表格编进二进制 |
| **源文件数** | 590 个 `.c/.h` | 115 个 `.rs` | Rust 工作区源码文件更少，但自动化覆盖更广 |
| **关卡定义** | 167 个 `.lua` 脚本 | 65 个 `.toml` + Rust 生成器 | TOML 存数据，Rust 写逻辑 |
| **数据文件** | 编译时表格 + dat 资源 | 65 TOML + 20 个 locale 文件 + 内嵌文本 | 发布产物现在是单一可执行文件 |
| **测试** | 约 0 个（纯人工 QA） | **4,677** 个自动化测试 | 单元 + 集成 + 性质 + 差异执行多层覆盖 |
| **机制规格** | 代码内注释 | **29 份规格文档** | 从原版提取的公式与测试向量 |
| **国际化** | 可选 `#ifdef` | 内建（5 种语言） | Fluent + TOML，运行时热切换 |
| **架构** | 全局状态，IO 混杂 | ECS + 零 IO 事件 | 确定性、可测试、可回放 |
| **二进制大小** | ~4MB | ~5.8MB | 单一可执行文件，内嵌数据与语言包 |

## 概述

NetHack Babel 是 [NetHack 3.7](https://github.com/NetHack/NetHack) 的 Rust 从零重写版本，保持公式级别的精确度，同时采用现代架构。它用 hecs ECS 替代原版的全局状态和手动内存管理，将游戏逻辑与所有 IO 完全分离，并将所有游戏内容——怪物、物品、地牢——定义在 TOML 数据文件中而非编译时表格。引擎发出类型化事件而非格式化字符串，通过 Project Fluent 实现内建多语支持（英文、简体中文、繁体中文、德文、法文），无需修改任何游戏逻辑。

## 当前代码规模

以下统计基于 2026-03-20 的仓库实测：

- **115 个 Rust 源文件 / 211,355 行**，覆盖 6 个 workspace crate
- **20 个 locale 文件 / 13,938 行**，包含 Fluent 与翻译 TOML
- **65 个数据 TOML 文件 / 25,639 行**，覆盖怪物、物品、关卡与 manifest
- **320 个已跟踪文件 / 305,703 行** 仓库总规模
- **29 份机制规格 + 29 份 review notes**，位于 `specs/` 与 `specs/reviews/`
- **4,677 个自动化测试**
- **394 个怪物定义 / 430 个物品定义**

## 原版 NetHack 简介

NetHack 是 Roguelike 史上最具代表性的作品之一，由 NetHack DevTeam 持续维护了数十年。它是回合制单机地牢探索游戏，以“系统互相组合”著称：物品、怪物、地形、状态之间会产生大量涌现玩法，再配合永久死亡（permadeath），使每一步决策都很关键。经典目标是取回延多护身符，穿越元素位面，并在星界正确祭坛献祭。NetHack Babel 的定位就是以 NetHack 3.7 行为为基准，尽可能做到机制级一致。

## 特性

- **真彩色终端渲染** — 基于 ratatui 的 TUI，16 色系统，BUC 着色背包，状态栏高亮
- **内建 5 语支持** — 英文、简体中文、繁体中文、德文、法文，运行时 `O` 键热切换
- **CJK 物品命名** — 量词系统："3把匕首"而非"3 daggers"；BUC 前缀："祝福的+2长剑"
- **数据驱动架构** — 394 种怪物、430 种物品、33 件神器，TOML 定义，无需重编译即可修改
- **ECS 游戏状态** — hecs 实体组件系统，显式回合解算与类型化事件
- **公式级精确** — 29 份机制规格从原版 C 源码提取；4,677+ 个测试验证忠实度
- **广覆盖可玩性** — 主线战役、特殊关卡、职业任务、终局、商店、宗教、宠物、陷阱、变形、骑乘、遗骨、回放与存读档都已是 live runtime
- **逐局外观洗牌** — 每局游戏随机化药水颜色、卷轴标签、戒指材质
- **完整特殊关卡** — 30+ 生成器：推箱子（8 关）、城堡、美杜莎、冥界全部恶魔巢穴、弗拉德之塔、巫师塔、至圣所、元素位面、星界、13 个职业任务
- **13 个可玩职业** — 考古学家、蛮人、穴居人、医者、骑士、武僧、祭司、游侠、盗贼、武士、旅行者、女武神、巫师——各有独特任务、初始装备和头衔
- **跨平台** — macOS、Linux、Windows
- **存读档与反 SL** — bincode 序列化，版本化头部
- **遗骨系统** — 死亡后留下幽灵和被诅咒的物品
- **排行榜** — JSON 持久化 Top 100 得分榜
- **确定性回放** — 显式 RNG 传递；相同种子 + 相同输入 = 相同游戏
- **巫师模式** — 调试命令：创建怪物、许愿、揭示地图、传送

## 快速开始

官方 release 现在按 tag 发布为**单一可执行文件**，不再需要额外拷贝 `data/` 目录。只有源码 checkout 运行时才继续使用仓库里的 `data/` 树。

### 直接运行 Release 二进制

```sh
./nethack-babel-v0.1.4-aarch64-apple-darwin --language zh-CN
```

### 从源码运行

```sh
git clone https://github.com/SatoshiNakamoto0126/nethack-babel.git
cd nethack-babel
cargo run -- --data-dir data --language zh-CN
```

需要 Rust nightly 的只有源码构建；通过 `rust-toolchain.toml` 会自动选择。使用 [rustup](https://rustup.rs/) 无需手动设置。

### 巫师模式（调试）

```sh
cargo run -- --data-dir data -D
```

使用 Ctrl+W（许愿）、Ctrl+F（揭示地图）、Ctrl+G（创建怪物）、Ctrl+I（全鉴定）。

## 操作按键

| 按键 | 动作 | 按键 | 动作 |
|------|------|------|------|
| `h` `j` `k` `l` | 移动（vi 键） | `y` `u` `b` `n` | 对角移动 |
| 方向键 | 移动（四方向） | `.` | 等待一回合 |
| `i` | 查看背包 | `,` | 拾取物品 |
| `d` | 丢弃物品 | `e` | 进食 |
| `q` | 饮用药水 | `r` | 阅读（卷轴） |
| `z` | 使用魔杖 | `Z` | 施放法术 |
| `f` | 射击（远程） | `t` | 投掷 |
| `w` | 装备武器 | `W` | 穿戴护甲 |
| `T` | 脱下护甲 | `P` | 佩戴饰品 |
| `R` | 摘下饰品 | `s` | 搜索 |
| `<` | 上楼 | `>` | 下楼 |
| `o` | 开门 | `c` | 关门 |
| `k` | 踢 | `a` | 使用（工具） |
| `p` | 向店主付款 | `O` | 选项 / 切换语言 |
| `S` | 保存并退出 | `Ctrl+C` | 退出 |
| `?` | 帮助 | `Ctrl+P` | 消息历史 |
| `#` | 扩展命令 | `F` | 强制攻击 |

### 扩展命令

按 `#` 后输入命令名（Tab 自动补全）：

| 命令 | 动作 | 命令 | 动作 |
|------|------|------|------|
| `pray` | 祈祷 | `loot` | 搜刮容器 |
| `enhance` | 提升武器技能 | `name` | 命名物品 |
| `dip` | 浸泡物品 | `ride` | 骑乘 |
| `offer` | 在祭坛献祭 | `invoke` | 激活神器 |
| `sit` | 坐下 | `jump` | 跳跃 |
| `turn` | 退散不死族 | `untrap` | 拆除陷阱 |
| `wipe` | 擦脸 | `swap` | 切换武器 |
| `known` | 已鉴定物品 | `vanquished` | 已击杀怪物 |
| `conduct` | 查看操守 | `overview` | 地牢概览 |
| `wait` | 等待 | `call` | 命名物品类别 |

## 架构

六个 Cargo crate 的工作空间，依赖关系严格单向；引擎不执行任何 IO。

| Crate | 职责 | 代码行数 | 测试数 |
|-------|------|---------|--------|
| `engine` | 纯游戏逻辑——战斗、怪物、物品、地牢、回合循环，80+ 个模块 | 171,220 | 3,800+ |
| `data` | TOML 数据定义与加载器：怪物、物品、地牢、关卡 | 4,753 | 44 |
| `i18n` | 基于 Fluent 的本地化、物品命名（doname）、CJK 量词 | 7,209 | 91 |
| `tui` | 基于 ratatui + crossterm 的终端界面，16 色系统 | 8,794 | 172 |
| `audio` | 基于 rodio 的音效，由引擎事件触发 | 340 | 58 |
| `cli` | 可执行文件入口——配置、存读档、选项、主循环 | 19,039 | 195 |

## 游戏系统

| 系统 | 状态 | 主要特性 |
|------|------|---------|
| 近战/远程战斗 | 完成 | 完整命中/伤害链、负 AC、背刺、双武器、武僧武术 |
| 法术（40+种） | 完成 | 火球、吸取生命、死亡之指、治疗、探测、变形等全效果 |
| 魔杖（24 种） | 完成 | 射线反弹、自我使用、物体命中、充能爆炸 |
| 药水（26 种）/ 卷轴（23 种） | 完成 | 完整 BUC 矩阵，投掷泼溅效果 |
| 神器（33 件） | 完成 | 特殊攻击、防御、激活效果 |
| 装备系统 | 完成 | 12 槽位，50+ 种内在属性（戒指/护符/靴子/斗篷/头盔/手套） |
| 怪物 AI | 完成 | 逃跑、施法、物品使用、恶魔领主特殊行为 |
| 地牢（8 分支） | 完成 | 主地牢、矿山、推箱子、任务、堡垒、冥界、弗拉德、终局 |
| 30+ 特殊关卡 | 完成 | 冥界恶魔巢穴、巫师塔、至圣所、元素位面、星界 |
| 13 个任务集 | 完成 | 职业专属地图、领袖、宿敌、神器 |
| 13 种房间 | 完成 | 商店、神殿、动物园、军营、蜂巢、太平间等 |
| 商店系统 | 完成 | 定价、魅力、价格鉴定、盗窃、信用、基石警察 |
| 宗教/祈祷 | 完成 | 祈祷、献祭、加冕、仆从召唤、守护天使 |
| 宠物系统 | 完成 | 忠诚度、饥饿、战斗 AI、跨层跟随 |
| 饥饿/进食 | 完成 | 尸体内在属性（16+ 种）、锡罐、噎食、同类相食 |
| 变形 | 完成 | 形态能力、系统震荡、装备破坏、下马 |
| 骑乘 | 完成 | 坐骑速度、战斗修正、水域通行 |
| 状态效果 | 完成 | 25+ 种计时效果，石化/黏化/疾病递进与死亡 |
| 陷阱（25 种） | 完成 | 全类型，玩家 + 怪物交互 |
| 爆炸 | 完成 | 7 种类型，3×3 范围，物品销毁 |
| 铁球与铁链 | 完成 | 拖拽机制、惩罚 |
| 吊桥 | 完成 | 状态机、实体压碎/闪避 |
| 金库/祭司/仆从 | 完成 | 守卫 NPC、神殿机制、神圣召唤 |
| 探测/光源/视野 | 完成 | 搜索、魔法地图、光源燃料、阴影投射 |
| 遗骨系统 | 完成 | 幽灵生成、物品诅咒 |
| 操守追踪 | 完成 | 13 标准 + 不刻 Elbereth |
| 得分与排行榜 | 完成 | JSON 持久化 Top 100，DYWYPI 披露 |
| 鉴定 | 完成 | 多层（外观、BUC、附魔），逐局洗牌外观 |
| 物品命名 | 完成 | doname/xname/an/the/复数/侵蚀 管线 |
| 属性锻炼 | 完成 | 力/敏/体/智/感/魅 从行动中增长 |
| 选项系统 | 完成 | 40+ 设置，RC 文件解析 |
| 帮助系统 | 完成 | 4 个帮助文件，符号查询 |
| 音乐/声音 | 完成 | 全部乐器，怪物发声，环境音效 |

## 国际化系统

NetHack Babel 将所有玩家可见文本与游戏逻辑分离：

- **1,624 条消息模板** — [Project Fluent](https://projectfluent.org/) `.ftl` 文件
- **实体名称翻译** — 各语言的怪物/物品名称 TOML 文件
- **量词** — `classifiers.toml` 用于 CJK 量词映射
- **物品命名** — `doname()` 管线产生"祝福的+2长剑"（中文）或 "a blessed +2 long sword"（英文）
- **热切换** — 游戏内按 `O` 切换，无需重启

## 构建

```sh
cargo build                              # 构建
cargo test --workspace                   # 运行全部 4,677+ 个测试
cargo run -- --data-dir data             # 运行游戏（英文）
cargo run -- --data-dir data --language zh-CN  # 运行游戏（简体中文）
cargo run -- --data-dir data -D          # 巫师模式
cargo clippy --workspace --all-targets   # 代码检查
cargo build --release                    # 发布构建
```

## 项目状态

项目现在已经进入“长尾保真”阶段，而不是“核心系统缺失”阶段。主线战役、职业任务、特殊关卡、终局、商店、宗教、宠物、遗骨、回放、存读档、排行榜和多语言 TUI 都已经可玩，并由 4,677 个自动化测试持续覆盖。当前剩余差距主要集中在原版 NetHack 的细节纹理和节奏感，例如 Wizard of Yendor 的 covetous 行为、`sounds.c` 的聊天与环境声长尾分支，以及少量选择型交互的 UX。

详见 [GAP_STATUS.md](GAP_STATUS.md)（状态报告）和 [DIFFERENCES.md](DIFFERENCES.md)（已知差异）。

## 项目路线图（2026）

1. **Wizard of Yendor 保真补完**  
   继续对齐 `wizard.c` 的 covetous targeting、`intervene()` 节奏和 `nasty()` 候选池，让后期压迫感更接近原版。
2. **NPC 发声与环境纹理补完**  
   继续对齐 `sounds.c`，优先收 `MS_VAMPIRE`、`MS_IMITATE`、`MS_TRUMPET`、full-moon/were 节奏，以及 swamp/barracks 等环境声。
3. **商店 / Oracle / 恶魔交互 UX**  
   底层机制已较完整，下一步重点是把咨询、勒索、报价、警告这些玩家可见交互做得更像原版 C 版。
4. **Drift harness 扩张**  
   继续扩 story/save matrix，把 wizard、宗教、经济和 branch state 的长尾分支纳入稳定回归。

## TODO（近期）

- [ ] 收完 `wizard.c` 剩余的 covetous / intervention 长尾。
- [ ] 收完 `sounds.c` 最高信号的聊天与环境声缺口。
- [ ] 把商店 / Oracle / 恶魔交互的提示流做得更接近原版。
- [ ] 继续把这些长尾分支收进 story/save drift harness。

## 文档

- [Player's Guidebook (English)](doc/GUIDEBOOK.md) — 完整英文游戏攻略
- [玩家指南（中文）](doc/GUIDEBOOK_zh.md) — 完整中文游戏攻略
- [DIFFERENCES.md](DIFFERENCES.md) — 与 C 版的架构差异
- [CONTRIBUTING.md](CONTRIBUTING.md) — 构建、代码风格、测试

## 许可证

[NetHack 通用公共许可证 (NGPL)](LICENSE)。

第三方 crate 依赖限制为 MIT、Apache-2.0、BSD 或 LGPL 许可证。

## 致谢

NetHack Babel 建立在 [NetHack 开发团队](https://nethack.org/) 数十年工作的基础之上。原版 NetHack 源码：[github.com/NetHack/NetHack](https://github.com/NetHack/NetHack)。
